...
//                        if ($today >= $ngayketthuc && $ngayketthuc >= $date_xuly) {
     ...
     * @param $html
     */
    public static function exportToWord($request){

        $arrDataDon = (new DonThuTable())->getAllInforDon($request->donthuid);
        if(!empty($arrDataDon[0]['sothuly']))
        {
            $name = explode('/',$arrDataDon[0]['sothuly']);
            $folder_name ="donthu_".$name[0]."_".$name[1];
        }
        else
        {
        ...
        $arrDataDon[0]['folder_name'] = "/file/".$folder_name;

        $arrDataDon[0]['gt'] = 'bà';
        if ($arrDataDon[0]['gioitinh'] == DonThuTable::GT_NAM) {
            $arrDataDon[0]['gt'] = 'ông';
        }

        $arrDataDon[0]['noiDung'] = $request->noiDung;
        $arrDataDon[0]['kinh_gui'] = $request->tieuDe;
        $arrDataDon[0]['soKH'] = $request->soKH;
        $arrDataDon[0]['ten_vb'] = MauDon::$arryVanBan[$request->loaiVB];
        $arrDataDon[0]['template'] = MauDon::$arrTemplateDocx[$request->loaiVB];
        $arrDataDon[0]['loai_vb'] = $request->loaiVB;
        $arrDataDon[0]['txtYkdx'] = $request->txtYkdx;
        $arrDataDon[0]['accountId'] = $request->accountId;



        $result = (new MauDon())->creatFile($arrDataDon);
     ...
     * @return string
     */
    public function postDonThuToCSDLQG($donId)
    {
        $arg = (new SysCSDLQGArgService())->SysArgToCreate($donId);

        $err = '';
        if (!empty($arg))
        {
            $res = (new CSDLQGService())->CapNhatVuViec($arg);
            $output = json_decode($res, true);

            if($output['Id']!= "")
            {
                $resUpdate = (new DonThuTable())->_update($donId,['id_csdlqg'=>$output['Id']]);
                ...
                $err = 'Create to CSDLQG error: '.$output['Message'];
            }
