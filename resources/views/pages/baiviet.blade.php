    ...
    <div id="donthu" class="col-background" style="margin-bottom: 100px;">
        <div class="panel panel-default panel-min-height">
            ...
            <div class="panel-body row">
                <div class="col-xs-8">
                    <a role="button" href="{{url('/themtintucsukien')}}" class="btn btn-success btn-sm" title="tạo đơn" id="btnThem"><span class="glyphicon glyphicon-plus"></span>Thêm</a>
                </div>
            ...
            <table class="table table-bordered table-hover" style="border-collapse: collapse">
                ...
                @foreach($gettintucsukien as $baiviet)
            ...
            <div class="panel-body text-center">
                <ul class="pagination">
                    <li>
                        <span>Trang {{$gettintucsukien->currentPage().'/'.$gettintucsukien->lastPage()}}</span>
                    </li>
                    <li @if($gettintucsukien->currentPage() == 1) class="disabled" @endif>
                        <a @if($gettintucsukien->currentPage() != 1) href="{{$gettintucsukien->url(1)}}" @endif>
                            <<
                        </a>
                    </li>
                    <li @if($gettintucsukien->currentPage() == 1) class="disabled" @endif>
                        <a @if($gettintucsukien->currentPage() != 1) href="{{$gettintucsukien->previousPageUrl()}}" @endif>
                            <
                        </a>
                    </li>
                    <li class="active">
                        <span>{{$gettintucsukien->currentPage()}}</span>
                    </li>
                    <li @if($gettintucsukien->currentPage() == $gettintucsukien->lastPage()) class="disabled" @endif>
                        <a @if($gettintucsukien->currentPage() != $gettintucsukien->lastPage()) href="{{$gettintucsukien->nextPageUrl()}}" @endif>
                            >
                        </a>
                    </li>
                    <li @if($gettintucsukien->currentPage() == $gettintucsukien->lastPage()) class="disabled" @endif>

                        <a @if($gettintucsukien->currentPage() != $gettintucsukien->lastPage()) href="{{$gettintucsukien->url($gettintucsukien->lastPage())}}" @endif>
                            >>
                        </a>
...
//                            alert(response['changetrangthaibaiviet_result']);
                        }
                    });
                }
...
//                        alert(response['xoabaiviet_result']);
        ...
        function chonHienThiSoLuong(valueChon){
            ...
            var baiVietUrl = <?php echo json_encode(url('baiviet')) ;?>;
...
//                    console.log(response);
                    window.location.href = baiVietUrl;
