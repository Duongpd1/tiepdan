...
@section('content')
    <script>
        ...
        var element1 = document.getElementById("phapluat");
    ...
    <div id="donthu" class="col-background" style="margin-bottom: 100px;">
        <div class="panel panel-default panel-min-height">
            <div class="panel-heading text-center">QUẢN TRỊ TRỢ GIÚP PHÁP LUẬT</div>
            ...
            <table class="table table-bordered table-hover" style="border-collapse: collapse">
                ...
                <tbody>
                @foreach($gettrogiupphapluat as $gopy)
                    <tr id="gopy{{$gopy->id}}">
                        <td>{{$gopy->hoten}}</td>
                        <td>{{$gopy->diachi}}</td>
            ...
            <div class="panel-body text-center">
                <ul class="pagination">
                    <li>
                        <span>Trang {{$gettrogiupphapluat->currentPage().'/'.$gettrogiupphapluat->lastPage()}}</span>
                    </li>
                    <li @if($gettrogiupphapluat->currentPage() == 1) class="disabled" @endif>
                        <a @if($gettrogiupphapluat->currentPage() != 1) href="{{$gettrogiupphapluat->url(1)}}" @endif>
                            <<
                        </a>
                    </li>
                    <li @if($gettrogiupphapluat->currentPage() == 1) class="disabled" @endif>
                        <a @if($gettrogiupphapluat->currentPage() != 1) href="{{$gettrogiupphapluat->previousPageUrl()}}" @endif>
                            <
                        </a>
                    </li>
                    <li class="active">
                        <span>{{$gettrogiupphapluat->currentPage()}}</span>
                    </li>
                    <li @if($gettrogiupphapluat->currentPage() == $gettrogiupphapluat->lastPage()) class="disabled" @endif>
                        <a @if($gettrogiupphapluat->currentPage() != $gettrogiupphapluat->lastPage()) href="{{$gettrogiupphapluat->nextPageUrl()}}" @endif>
                            >
                        </a>
                    </li>
                    <li @if($gettrogiupphapluat->currentPage() == $gettrogiupphapluat->lastPage()) class="disabled" @endif>

                        <a @if($gettrogiupphapluat->currentPage() != $gettrogiupphapluat->lastPage()) href="{{$gettrogiupphapluat->url($gettrogiupphapluat->lastPage())}}" @endif>
                            >>
                        </a>
    ...
    <script>
        ...
        function chonHienThiSoLuong(valueChon){
            ...
            var qttrogiupphapluatUrl = <?php echo json_encode(url('qttrogiupphapluat')) ;?>;
...
//                    console.log(response);
                    window.location.href = qttrogiupphapluatUrl;
